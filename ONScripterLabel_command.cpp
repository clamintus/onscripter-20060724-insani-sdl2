...
#if !defined(PSP)
    #ifdef SDL2
        window_title = SDL_GetWindowTitle( window );
...
int ONScripterLabel::menu_fullCommand()
{
    if ( !fullscreen_mode ){
        ...
        #ifdef SDL2
            SDL_DestroyWindow( window );
            window = SDL_CreateWindow( window_title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, screen_width, screen_height, SDL_WINDOW_FULLSCREEN );
            screen_surface = SDL_GetWindowSurface( window );
        #else
            SDL_FreeSurface(screen_surface);
...
int ONScripterLabel::btndefCommand()
{
    if (script_h.compareString("clear")){
        ...
        #ifdef SDL2
            if ( SDL_SetSurfaceAlphaMod( btndef_info.image_surface, SDL_ALPHA_OPAQUE ) != 0 )
                fprintf( stderr, "%s\n", SDL_GetError() );
        #else
