...
#ifdef SDL2
    window = SDL_CreateWindow("ONScripter-SDL2",
...
#else
    screen_surface = SDL_SetVideoMode( screen_width, screen_height, screen_bpp, DEFAULT_VIDEO_SURFACE_FLAG|(fullscreen_mode?SDL_WINDOW_FULLSCREEN:0) );
#endif
...
#ifdef SDL2
    if ( SDL_SetSurfaceBlendMode( accumulation_surface, SDL_BLENDMODE_NONE ) != 0 )
        fprintf( stderr, "%s\n", SDL_GetError() );
    if ( SDL_SetSurfaceBlendMode( effect_src_surface, SDL_BLENDMODE_NONE ) != 0 )
        fprintf( stderr, "%s\n", SDL_GetError() );
    if ( SDL_SetSurfaceBlendMode( effect_dst_surface, SDL_BLENDMODE_NONE ) != 0 )
        fprintf( stderr, "%s\n", SDL_GetError() );
#else
...
void ONScripterLabel::flushDirect( SDL_Rect &rect, int refresh_mode )
{
    ...
    if ( SDL_BlitSurface( accumulation_surface, &rect, screen_surface, &rect ) != 0 )
        SDL_Quit();
#ifdef SDL2
